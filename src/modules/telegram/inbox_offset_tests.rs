...
async fn fixture() -> (TempDir, SqlitePool) {
    ...
    sqlx::query(
        "INSERT INTO telegram_bot_offsets (bot_id, next_offset, updated_at) VALUES (?, 100, ?)",
    )
    .bind(BOT)
    .bind(NOW)
    .execute(&pool)
...
async fn assert_state(pool: &SqlitePool, memory: i64, offset: i64, ids: &[i64]) {
    assert_eq!(
        memory, offset,
        "in-memory offset must reflect a committed batch"
    );
    ...
    assert_eq!(
        actual, ids,
        "a failed batch must not leave partial inbox rows"
    );
}
...
async fn rollback_on_offset_write_failure() {
    ...
    let mut offset = 100;
    assert!(persist_update_batch(&pool, BOT, batch.clone(), &mut offset)
        .await
        .is_err());
    assert_state(&pool, offset, 100, &[]).await;
    sqlx::query("DROP TRIGGER reject_test_offset")
        .execute(&pool)
        .await
        .unwrap();
    assert_eq!(
        persist_update_batch(&pool, BOT, batch, &mut offset)
            .await
            .unwrap()
            .len(),
        2
...
async fn rollback_on_deferred_commit_failure() {
    ...
    assert_eq!(
        guards, 0,
        "failed COMMIT must roll back its trigger effects too"
    );
...
async fn rollback_on_invalid_update_id() {
    for invalid in [-1, i64::MAX] {
        ...
        let error = persist_update_batch(
            ...
            BOT,
            vec![
                update(100, "valid first"),
                update(invalid, "invalid second"),
            ],
            &mut offset,
        )
        .await
...
async fn successful_batch_and_replay_keep_status_filtering() {
    ...
    let batch = vec![
        update(103, "failed"),
        update(100, "received"),
        update(102, "processed"),
        update(101, "processing"),
    ];
    let mut offset = 100;
    let first = persist_update_batch(&pool, BOT, batch.clone(), &mut offset)
        .await
        .unwrap();
    assert_eq!(
    ...
    let replay = persist_update_batch(&pool, BOT, batch, &mut offset)
        .await
        .unwrap();
    assert_eq!(
        replay,
        vec![(103, update(103, "failed")), (100, update(100, "received"))]
    );
    assert_state(&pool, offset, 104, &[100, 101, 102, 103]).await;
}
...
async fn empty_and_missing_ids_keep_existing_skip_contract() {
    ...
    for batch in [
        ...
        assert!(persist_update_batch(&pool, BOT, batch, &mut offset)
            .await
            .unwrap()
            .is_empty());
        assert_state(&pool, offset, 100, &[]).await;
