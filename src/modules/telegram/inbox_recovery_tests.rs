...
async fn state(pool: &SqlitePool, id: i64) -> (String, i64) {
    sqlx::query_as(
        "SELECT status, attempt_count FROM telegram_updates WHERE bot_id = ? AND update_id = ?",
    )
    .bind(BOT)
    .bind(id)
    .fetch_one(pool)
...
async fn seed(pool: &SqlitePool, id: i64, status: &str, attempts: i64, timestamp: &str) {
    ...
    .bind(BOT)
    .bind(id)
    .bind(json!({"update_id": id}).to_string())
    .bind(status)
    .bind(attempts)
    .bind(timestamp)
    .bind(timestamp)
...
async fn due_selection_has_exact_backoff_boundaries_and_a_bounded_page() {
    ...
    assert!(pending_inbox_batch(&pool, BOT, 946684801)
        .await
        .unwrap()
        .is_empty());
    assert_eq!(
        ids(pending_inbox_batch(&pool, BOT, 946684802).await.unwrap()),
        [1]
    );
    assert_eq!(
        ids(pending_inbox_batch(&pool, BOT, 946684804).await.unwrap()),
        [1, 2]
    );
    assert!(pending_inbox_batch(&pool, "other-bot", i64::MAX)
        .await
        .unwrap()
        .is_empty());
    for id in 10..115 {
...
async fn malformed_row_does_not_starve_later_valid_updates() {
    ...
    let code: String =
        sqlx::query_scalar("SELECT error_summary FROM telegram_updates WHERE update_id = 1")
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn failed_inbox_recovers_in_the_same_poller_with_no_new_updates() {
    let (_dir, pool) = super::inbox_offset_tests::fixture().await;
    // A startup reset failure must be logged, not terminate the poller and
    ...
    sqlx::query(
        "CREATE TRIGGER reject_startup_reset BEFORE UPDATE OF status ON telegram_updates
    ...
    sqlx::query(
        "CREATE TRIGGER reject_inbox_completion BEFORE UPDATE OF status ON telegram_updates
    ...
    Mock::given(method("POST"))
        .respond_with(
            ResponseTemplate::new(200).set_body_json(
                json!({"ok": true, "result": {"id": 123, "username": "synthetic_bot"}}),
            ),
        )
        .mount(&server)
        .await;
    ...
    let task = tokio::spawn(poll_telegram_bot_at(
    ...
    let result = tokio::time::timeout(Duration::from_secs(15), async {
        loop {
            let failed: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM telegram_updates WHERE update_id = 100 AND status = 'failed'",
            )
            .fetch_one(&pool)
            .await
            .unwrap();
        ...
        sqlx::query("DROP TRIGGER reject_inbox_completion")
            .execute(&pool)
            .await
            .unwrap();
        loop {
            if state(&pool, 100).await.0 == "processed" {
                break;
            }
    ...
    assert_eq!(state(&pool, 99).await, ("processing".into(), 1));
