...
impl Fixture {
    ...
    fn managed(&self, version: u32) -> OperationCoordinator {
        let vault: Arc<dyn SecretVault> =
            Arc::new(EncryptedSqliteVault::new(self.app.pool.clone(), [0x71; 32]));
...
fn assert_error(
    result: AppResult<OperationRecoveryPayload>,
    code: &str,
    status: u16,
    message: &str,
) {
    ...
    assert_eq!(
        (error.code, error.status.as_u16(), error.message.as_str()),
        (code, status, message)
    );
}
...
async fn fixed_entrypoints_accept_all_variants_with_and_without_digest() {
    ...
    for payload in fixture.payloads() {
        ...
        for has_digest in [true, false] {
            ...
            assert_eq!(
                coordinator
                    .decrypt_recovery_payload(&record, "bot-c03", &fixture.fence)
                    .unwrap(),
                payload
            );
            assert_eq!(
                coordinator
                    .decrypt_recovery_payload_for_operation(&record, &fixture.fence)
                    .await
                    .unwrap(),
                payload
            );
...
async fn entry_specific_scope_rejections_preserve_messages() {
    ...
    for bot in ["", " ", "other-bot"] {
        assert_error(
            coordinator.decrypt_recovery_payload(&record, bot, &fixture.fence),
            "PAYLOAD_AAD_INVALID",
            409,
            "operation payload runtime fence does not match the historical operation",
        );
    ...
    for index in 0..12 {
        ...
        match index {
            ...
            _ => {
                ...
                let coordinator = fixture.fixed(Some(fixture.key.clone()));
                assert_error(
                    coordinator.decrypt_recovery_payload(&record, "other-bot", &fence),
...
async fn cipher_nonce_key_and_version_tampering_is_rejected() {
    ...
    let record = fixture.encrypted(&fixture.payloads()[2]);
    let wrong = fixture.fixed(Some(Arc::new(OperationPayloadEncryptor::new(
        [0x54; 32], 7,
    ))));
    assert_error(
        wrong.decrypt_recovery_payload(&record, "bot-c03", &fixture.fence),
        "PAYLOAD_DECRYPT_FAILED",
        400,
        "operation payload could not be decrypted",
    );
...
async fn invalid_payload_is_rejected_before_digest() {
    ...
    for index in 0..4 {
        ...
        record.mutation_digest = Some("wrong-digest".into());
        fixture
            .rejected(&record, &fixture.fence, code, 400, message)
            .await;
    }
    let mut payload = fixture.payloads().pop().unwrap();
    if let OperationRecoveryPayload::Create { command } = &mut payload {
...
async fn managed_restore_uses_persisted_key_version_without_writes() {
    ...
    let key = writer
        .payload_encryptor_for_operation(&fixture.record.id)
        .await
        .unwrap();
    ...
    fixture
        .store
        .update_generated(&record.id, record.payload.as_ref().unwrap(), &digest)
        .await
        .unwrap();
    drop(writer);
    drop(key);
    let stored = fixture
        .store
        .get_operation(&record.id)
        .await
        .unwrap()
        .unwrap();
    let before = fixture
        .store
        .get_operation_key_reference(&record.id)
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        restored
            .decrypt_recovery_payload_for_operation(&stored, &fixture.fence)
            .await
            .unwrap(),
        payload
    );
    ...
    assert_error(
        restored.decrypt_recovery_payload(&stored, "bot-c03", &fixture.fence),
        "PAYLOAD_KEY_UNAVAILABLE",
        503,
        "operation payload key is unavailable",
    );
    ...
    assert_error(
        restored
            .decrypt_recovery_payload_for_operation(&tampered, &fixture.fence)
            .await,
        "PAYLOAD_DIGEST_MISMATCH",
        409,
        "operation recovery payload digest mismatch",
...
async fn missing_reference_never_creates_a_key_and_precedes_payload_validation() {
    ...
    for record in [
        ...
        fixture.encrypted(&fixture.payloads()[2]),
    ] {
        assert_error(
            managed
                .decrypt_recovery_payload_for_operation(&record, &invalid_fence)
                .await,
