...
async fn capture_upload(
    ...
    let content_type = headers[axum::http::header::CONTENT_TYPE]
        .to_str()
        .unwrap()
        .to_owned();
    ...
    async fn trusted_certificate_succeeds_but_unknown_root_and_wrong_name_fail() {
        ...
        let client = |trusted: bool| {
            let builder = reqwest::Client::builder()
                ...
                .resolve("wrong.example.invalid", address);
            if trusted {
                builder
        ...
        assert!(
            format!("{unknown:?}").contains("UnknownIssuer"),
            "{unknown:?}"
        );
        let wrong_name = client(true)
            .get(format!("https://wrong.example.invalid:{}/", address.port()))
            .send()
            .await
            .unwrap_err();
